...
export async function GET(request: Request) {
  ...
  const [{ data: reservations, error: reservationError }, { data: admins, error: adminError }] = await Promise.all([
    admin
      .from("reservations")
      .select("id, checked_out_at, checkout_location, river_direction, boats(name), profiles!reservations_created_by_fkey(id,full_name,email)")
      .eq("status", "checked_out")
      .lte("checked_out_at", thresholdIso),
    admin.from("profiles").select("email").eq("role", "admin").eq("status", "active").neq("email", ""),
  ]);

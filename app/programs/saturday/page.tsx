  ...
  return (
    <>
      ...
      <main className="stack">
        <PageTitle title="Saturday Coached Row" subtitle={`Signups for ${month.label}`} />

        <div className="row">
          <Link href={`/programs/saturday?month=${month.prev}`}>Previous Month</Link>
          <Link href={`/programs/saturday?month=${month.next}`}>Next Month</Link>
        </div>

        <div className="stack">
          {sessions.length === 0 ? <Card subtle>No Saturday rows in this month yet.</Card> : null}

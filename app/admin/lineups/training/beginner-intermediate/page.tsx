...
export default async function AdminTrainingBILineupsPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  return (
    <>
      ...
      <main className="stack">
        <PageTitle title="Training BI Lineups" subtitle={month.label} />
        <div className="row">
          <Link href="/admin/lineups">Back</Link>
          <Link href={`/admin/lineups/training/beginner-intermediate?month=${month.prev}`}>Previous Month</Link>
          <Link href={`/admin/lineups/training/beginner-intermediate?month=${month.next}`}>Next Month</Link>
        </div>
